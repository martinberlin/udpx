...
Ideas are still not 100% defined but my personal wishes are: 

1. No more Protocol detection by the first two bytes of the received package. That was a Bullscheiße idea Martin!
2. 6 bytes header respecting the "p" starter and just space for one more byte (Also matches Spectre header)
3. No more Brotli. Since Zlib works much faster and also does not fail decompressing so often like the other
4. A very well documented protocol. No guess like. Very adaptable to LED-Master. We need to respect the master ;)
